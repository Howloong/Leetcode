...
//Java:设计一个验证系统
//Time:2023-02-09 09:31:43
class P1797_DesignAuthenticationManager {
...
class AuthenticationManager {
    ...
    public void renew(String tokenId, int currentTime) {
        int generateTime = tokenMap.getOrDefault(tokenId, -1);
        if (generateTime != -1 && currentTime - generateTime < timeToLive) {
            tokenMap.put(tokenId, currentTime);
        }
    }
    ...
    public int countUnexpiredTokens(int currentTime) {
        return (int) tokenMap.values().stream()
                .filter(thisTime -> currentTime - thisTime < timeToLive).count();
    }
}
