...
// 
//
// Related Topics 贪心 数组 字符串匹配 👍 18 👎 0

package leetcode.editor.cn;

//Java:通过连接另一个数组的子数组得到一个数组
//Time:2022-12-17 10:24:38
class P1764_FormArrayByConcatenatingSubarraysOfAnotherArray {
...
//        System.out.println(solution.canChoose(new int[][]{{10, -2}, {1, 2, 3, 4}}, new int[]{1, 2, 3, 4, 10, -2}));
    ...
    class Solution {
        public boolean canChoose(int[][] groups, int[] nums) {
            int k = 0;
            int count = 0;
            while (k < nums.length && count < groups.length) {
                if (check(groups[count], nums, k)) {
                    k += groups[count].length;
                    count++;
                } else {
                    k++;
                }
        ...
        public boolean check(int[] res, int[] nums, int k) {
            if (k + res.length > nums.length) {
                return false;
            }
            int j = 0;
