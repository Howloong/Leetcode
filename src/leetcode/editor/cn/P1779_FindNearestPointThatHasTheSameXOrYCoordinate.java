...
//Java:找到最近的有相同 X 或 Y 坐标的点
//Time:2022-12-01 10:11:05
...
//        int minIdx_X = Integer.MAX_VALUE;
//        int minIdx_Y = Integer.MAX_VALUE;
            int minIdx = Integer.MAX_VALUE;
/*            for (int[] point : points) {
                if (point[0] == x) {
                    int distance = Math.abs(y - point[1]);
                    if (distance == min && minIdx > x) {
                        minIdx = x;
                    ...
                    int distance = Math.abs(x - point[0]);
                    if (distance == min && minIdx > y) {
                        minIdx = y;
                    } else if (distance < min) {
                        min = distance;
                        minIdx = y;
