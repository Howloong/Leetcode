...
// 
//
// Related Topics 队列 数组 滑动窗口 单调队列 堆（优先队列） 👍 123 👎 0
...
//Java:满足不等式的最大值
//Time:2023-07-21 19:21:58
class P1499_MaxValueOfEquation {
    ...
    class Solution {
        public int findMaxValueOfEquation(int[][] points, int k) {
            ...
            for (int i = 0; i < points.length; i++) {
                while (!deque.isEmpty() && points[i][0] - points[deque.peekLast()][0] > k) {
                    deque.pollLast();
                }
                if (!deque.isEmpty()) {
                    res = Math.max(res, points[i][1] + points[deque.peekLast()][1] + points[i][0] - points[deque.peekLast()][0]);
                }
                while (!deque.isEmpty() && points[i][1] - points[i][0] > points[deque.peekFirst()][1] - points[deque.peekFirst()][0]) {
                    deque.pollFirst();
                }
                deque.push(i);
            }
