...
class P1974_MinimumTimeToTypeWordUsingSpecialTypewriter {
    public static void main(String[] args) {
        Solution solution = new P1974_MinimumTimeToTypeWordUsingSpecialTypewriter().new Solution();
    ...
    class Solution {
        public int minTimeToType(String word) {
            ...
            for (int i = 0; i < word.length(); i++) {
                ...
                if (Math.abs(c - nowChar) >= 13) {
                    sum = sum + 26 - Math.abs(c - nowChar);
                } else {
                    sum = sum + Math.abs(c - nowChar);
                }
                nowChar = c;
                sum += 1;
            }
