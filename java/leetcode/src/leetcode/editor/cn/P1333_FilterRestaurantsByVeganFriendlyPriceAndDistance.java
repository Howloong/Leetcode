...
//Java:餐厅过滤器
//Time:2023-09-27 15:21:57
class P1333_FilterRestaurantsByVeganFriendlyPriceAndDistance {
    public static void main(String[] args) {
        Solution solution = new P1333_FilterRestaurantsByVeganFriendlyPriceAndDistance().new Solution();
        System.out.println(solution.filterRestaurants(new int[][]{{1, 4, 1, 40, 10}, {2, 8, 0, 50, 5},
                ...
                for (int[] restaurant : restaurants) {
                    if (restaurant[2] == 1) {
                        if (restaurant[3] <= maxPrice && restaurant[4] <= maxDistance) {
                            res.add(new int[]{restaurant[0],restaurant[1]});
                        }
                    }
                }
            } else {
