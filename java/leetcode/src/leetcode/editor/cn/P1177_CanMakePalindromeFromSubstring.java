...
// 
//
// Related Topics 位运算 数组 哈希表 字符串 前缀和 👍 106 👎 0
...
//Java:构建回文串检测
//Time:2023-06-15 09:40:31
class P1177_CanMakePalindromeFromSubstring {
    public static void main(String[] args) {
        Solution solution = new P1177_CanMakePalindromeFromSubstring().new Solution();
        System.out.println(solution.canMakePaliQueries("abcda",
    ...
    class Solution {
        ...
        public List<Boolean> canMakePaliQueries(String s, int[][] queries) {
            ...
            for (int i = 0; i < s.length(); i++) {
                sum[i + 1] = sum[i] ^ (1 << (s.charAt(i) - 'a'));
            }
            for (int[] query : queries) {
                if (query[0] == query[1]) {
                    res.add(true);
                } else {
                    int t = sum[query[1] + 1] ^ sum[query[0]];
                    if (map.containsKey(t)) {
                        res.add(map.get(t) <= query[2]);
                    } else {
                        int m = func(sum[query[1] + 1] ^ sum[query[0]]);
                        map.put(t, m);
                        res.add(m <= query[2]);
                    }
                }
