...
//Java:拆分成最多数目的正偶数之和
//Time:2023-07-12 21:57:03
class P2178_MaximumSplitOfPositiveEvenIntegers {
    public static void main(String[] args) {
        Solution solution = new P2178_MaximumSplitOfPositiveEvenIntegers().new Solution();
    }
    ...
    class Solution {
        public List<Long> maximumEvenSplit(long finalSum) {
            ...
            long sum = 0;
            long t = 2;
            while (sum + t <= finalSum) {
                sum += t;
                res.add(t);
                t += 2;
            }
            Long remove = res.remove(res.size() - 1);
            res.add(remove + finalSum - sum);
            return res;
        }
    }
