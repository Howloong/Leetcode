...
// 
//
// Related Topics 设计 哈希表 数据流 有序集合 堆（优先队列） 👍 169 👎 0
...
class P2034_StockPriceFluctuation {
    ...
    class StockPrice {
        private final PriorityQueue<int[]> pqMax;
        private final PriorityQueue<int[]> pqMin;
        private final Map<Integer, Integer> map;
        private int maxTimeStamp;

        public StockPrice() {
            this.map = new HashMap<>();
            this.pqMax = new PriorityQueue<>(Comparator.comparing(o -> -o[0]));
            this.pqMin = new PriorityQueue<>(Comparator.comparing(o -> o[0]));
            this.maxTimeStamp = 0;
        }
        ...
        public int maximum() {
            while (true) {
                ...
                if (map.get(priceTime[1]) == priceTime[0]) {
                    return priceTime[0];
                }
                pqMax.poll();
            }
