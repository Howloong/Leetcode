...
// 
//
// Related Topics 数组 哈希表 字符串 计数 👍 143 👎 0
...
class P811_SubdomainVisitCount {
    ...
    class Solution {
        ...
        public void func(HashMap<String, Integer> hashMap, String domain) {
            String[] temp = domain.split(" ");
            int times = Integer.parseInt(temp[0]);
            String[] subDomains = temp[1].split("\\.");
            if (subDomains.length == 3) {
                String sub1 = subDomains[0] + "." + subDomains[1] + "." + subDomains[2];
                hashMap.put(sub1, hashMap.getOrDefault(sub1, 0) + times);
                String sub2 = subDomains[1] + "." + subDomains[2];
                hashMap.put(sub2, hashMap.getOrDefault(sub2, 0) + times);
                String sub3 = subDomains[2];
                hashMap.put(sub3, hashMap.getOrDefault(sub3, 0) + times);
            } else if (subDomains.length == 2) {
                String sub1 = subDomains[0] + "." + subDomains[1];
                hashMap.put(sub1, hashMap.getOrDefault(sub1, 0) + times);
                String sub2 = subDomains[1];
                hashMap.put(sub2, hashMap.getOrDefault(sub2, 0) + times);
            }
        }
    }
