...
//Java:给定行和列的和求可行矩阵
//Time:2023-03-14 09:29:19
class P1605_FindValidMatrixGivenRowAndColumnSums {
    public static void main(String[] args) {
        Solution solution = new P1605_FindValidMatrixGivenRowAndColumnSums().new Solution();
    ...
    class Solution {
        public int[][] restoreMatrix(int[] rowSum, int[] colSum) {
            ...
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    int value = Math.min(colSum[j], rowSum[i]);
                    res[i][j] = value;
                    colSum[j] -= value;
                    rowSum[i] -= value;
                }
