...
// 
//
// Related Topics 字符串 滑动窗口 👍 111 👎 0

package leetcode.editor.cn.java;

//Java:得到 K 个黑块的最少涂色次数
//Time:2023-03-10 11:55:46
class P2379_MinimumRecolorsToGetKConsecutiveBlackBlocks {
    ...
    class Solution {
        public int minimumRecolors(String blocks, int k) {
            ...
            int min = 0;
            while (right < k) {
                min += blocks.charAt(right) == 'W' ? 1 : 0;
                right++;
            }
            res = Math.min(res, min);
            while (right < blocks.length()) {
                min += blocks.charAt(right) == 'W' ? 1 : 0;
                min -= blocks.charAt(left) == 'W' ? 1 : 0;
