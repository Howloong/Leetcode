...
//Java:并行课程 III
//Time:2023-07-28 19:22:19
class P2050_ParallelCoursesIii {
    public static void main(String[] args) {
        Solution solution = new P2050_ParallelCoursesIii().new Solution();
    }
    ...
    class Solution {
        public int minimumTime(int n, int[][] relations, int[] time) {
            ...
            Arrays.setAll(lists, (i) -> lists[i] = new ArrayList<>());
            ...
            while (!deque.isEmpty()) {
                ...
                for (int i : lists[t]) {
                    in[i]--;
                    dp[i] = Math.max(dp[i], dp[t] + time[i]);
                    sum = Math.max(sum, dp[i]);
                    if (in[i] == 0) {
                        deque.offer(i);
                    }
